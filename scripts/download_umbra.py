...
Usage:
pixi run download -i paradise.geojson -a Metadata -o /tmp/paradise
./download_umbra.py -i utqiagvik.geojson -a GEC -o /tmp/utqiagvik
'''
import stac_asset
...
def search_stac_items(aoi: dict) -> pystac.ItemCollection:
    '''
    Search organization orders for a specific AOI
    '''
    limit_results=2000
...
def get_asset_key_map(item):
    '''
    Umbra's asset keys are file names rather than constant strings, so we have to look them up
    '''
    title2assetkey = {
        'Metadata': '',
        'CPHD': '',
...
def main(aoi: str, asset: str, outdir: str):
    with open(aoi) as f:
        feature_collection = json.load(f)
        aoi = feature_collection['features'][0]['geometry']
    items = search_stac_items(aoi)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--intersects', required=True, type=str, help='Path to a GeoJSON file for the area of interest')
    ...
    main(args.intersects, args.asset, args.outdir)
